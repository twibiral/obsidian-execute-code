...
export default class ExecuteCodePlugin extends Plugin {
	...
	private addRunButtons(element: HTMLElement, file: string) {
		Array.from(element.getElementsByTagName("code")).forEach(
			(codeBlock: HTMLElement) => {
				...
				const canonicalLanguage = getLanguageAlias(
					supportedLanguages.find((lang) =>
						codeBlock.classList.contains(`language-${lang}`)
					)
				) as LanguageId;

				if (
					canonicalLanguage && // if the language is supported
					!parent.classList.contains(hasButtonClass)
				) {
					// & this block hasn't been buttonified already
					const out = new Outputter(
						codeBlock,
	...
	private addListenerToButton(
		...
		if (language === "js") {
			button.addEventListener("click", async () => {
				...
				let transformedCode = await new CodeInjector(
					this.app,
					this.settings,
					language
				).injectCode(srcCode);
			...
			button.addEventListener("click", async () => {
				...
				const transformedCode = await new CodeInjector(
					this.app,
					this.settings,
					language
				).injectCode(srcCode);
			...
			button.addEventListener("click", async () => {
				...
				if (this.settings.pythonEmbedPlots)
					// embed plots into html which shows them in the note
